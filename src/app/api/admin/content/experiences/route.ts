...
export async function GET() {
  ...
  const data = readJsonFile("experiences.json", [] as any[]);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const items = readJsonFile("experiences.json", [] as any[]);
    const newItem = {
      id: `exp-${Date.now()}`,
      ...body,
    ...
    items.push(newItem);
    writeJsonFile("experiences.json", items);
    revalidatePath("/", "page");
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    writeJsonFile("experiences.json", filtered);
    revalidatePath("/", "page");
    revalidatePath("/portfolio", "page");
